...
bool SmtpClient::connectToHost()
{
	...
	try
	{
		...
		if (connectionType == TlsConnection) {
			...
			QSslSocket* ss = (QSslSocket*) socket;
			if( !_peerName.isEmpty() )
				ss->setPeerVerifyName( _peerName );
			if( !_peerCertificate.isNull() )
				ss->addCaCertificate( _peerCertificate );
...
void SmtpClient::setPeer( const QString& peerName, const QSslCertificate& peerCertificate ){
	_peerName = peerName;
